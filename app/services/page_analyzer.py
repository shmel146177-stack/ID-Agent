...
class PageAnalyzer:
    ...
    def __init__(self):
        ...
        self.rules = {
            ...
            "Ведомость рабочих чертежей": {
                ...
                "ведомость документов основного комплекта рабочих чертежей": 25,
    ...
    def _detect_priority_type(
        ...
        if is_drawing:
            if (
                "ведомость рабочих чертежей" in normalized_text
                or "ведомость документов основного комплекта рабочих чертежей"
                in normalized_text
            ):
        ...
        priority_rules = [
            ...
            (
                "Схема границ",
                ["схема границ на выдачу разрешения на размещение"],
            ),
            (
                "Документы СРО",
                [
            ...
            (
                "Техническое задание",
                [
                    "требования к техническим решениям и составу проекта",
                    "технические решения должны соответствовать требованиям",
                ],
            ),
            (
                "План благоустройства",
                ["план благоустройства", "план длагоустройства"],
            ),
            (
        ...
        if not is_drawing and "теплосеть" in normalized_text:
            return "Согласование"

        if (
            "восстановление газона" in normalized_text
            or "восстановление щебеночного покрытия" in normalized_text
        ) and "транш" in normalized_text:
            return "План благоустройства"

        if (
            "члена саморегулируемой организации" in normalized_text
            or "компенсационный фонд" in normalized_text
            or "сведения об обязательствах по договорам строительного подряда"
            in normalized_text
        ):
            return "Документы СРО"
